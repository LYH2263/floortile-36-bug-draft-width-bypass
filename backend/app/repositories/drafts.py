...
def confirm_draft(draft_id: int) -> dict:
    ...
    try:
        ...
        now = _now()
        # A draft confirms exactly once; any non-open status is a hard failure.
        ...
        room = dict(room)
        tile = dict(tile)
        # Any drift from the snapshot — room length, room width, or tile
        # dimensions — invalidates the draft.
        if (
        ...
        # Persist the draft's snapshot as-is; confirming never recomputes.
